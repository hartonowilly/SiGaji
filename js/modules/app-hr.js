...
function applyPayrollSpTabSimpleUi(){
  ...
  if(!wrap)return;
  wrap.querySelectorAll('.spt[data-kgstab]').forEach(function(el){
...
function renderTunjPanel(k){
  ...
  document.getElementById('tunj-list').innerHTML=list.length?list.map(function(t,i){
    ...
    return '<div class="tr-row payroll-tunj-row" style="grid-template-columns:1.25fr 1fr 1fr 0.58fr 0.72fr auto">'
      +'<input value="'+escapeAttr(t.nama)+'" placeholder="Nama tunjangan" style="padding:5px 8px;border:1.5px solid #dde1e9;border-radius:6px;font-size:12px;font-family:inherit;outline:none" data-nik="'+escapeAttr(nik)+'" data-i="'+i+'" data-f="nama" onchange="updTunjEl(this)">'
      +'<input type="text" class="inp-rp" value="'+t.nilai+'" placeholder="Nominal (Rp)" '+(snapMode?'':'readonly title="Nominal tunjangan diisi pada mode Snapshot Periode"')+' style="padding:5px 8px;border:1.5px solid #dde1e9;border-radius:6px;font-size:12px;font-family:inherit;outline:none;'+nilaiStyle+'" data-nik="'+escapeAttr(nik)+'" data-i="'+i+'" data-f="nilai" data-rp="1" onchange="updTunjEl(this)">'
      +'<select style="padding:5px 8px;border:1.5px solid #dde1e9;border-radius:6px;font-size:11px;font-family:inherit;outline:none" data-nik="'+escapeAttr(nik)+'" data-i="'+i+'" data-f="tipe" onchange="updTunjEl(this);refreshTunjPanel(this.dataset.nik)">'+sel1+'</select>'
      +'<select title="Ikut THR?" style="padding:5px 8px;border:1.5px solid #dde1e9;border-radius:6px;font-size:10px;font-family:inherit;outline:none;'+(showThr?'':'opacity:.35;pointer-events:none')+'" data-nik="'+escapeAttr(nik)+'" data-i="'+i+'" data-f="thr_ikut" onchange="updTunjEl(this)">'
        +'<option value="ya" '+(thrIkut?'selected':'')+'>&#127873;THR</option>'
        +'<option value="tidak" '+(!thrIkut?'selected':'')+'>Excl</option>'
      +'</select>'
      +'<select title="Ikut perhitungan prorata gaji?" style="padding:5px 8px;border:1.5px solid #dde1e9;border-radius:6px;font-size:10px;font-family:inherit;outline:none" data-nik="'+escapeAttr(nik)+'" data-i="'+i+'" data-f="prorata_ikut" onchange="updTunjEl(this)">'
        +'<option value="ya" '+(prIkut?'selected':'')+'>Prorata</option>'
        +'<option value="tidak" '+(!prIkut?'selected':'')+'>Penuh</option>'
      +'</select>'
      +'<button type="button" class="btn btn-xs btn-r"'+sigajiDataAction('del-tunj',{nik:nik,i:i})+'>&#10007;</button>'
    +'</div>';
...
function renderPotPanel(k){const list=k.potongan||[];document.getElementById('pot-list').innerHTML=list.length?list.map((p,i)=>`<div class="tr-row payroll-pot-row" style="grid-template-columns:2fr 1fr 1fr auto"><input value="${escapeAttr(p.nama)}" placeholder="Nama potongan" style="padding:5px 8px;border:1.5px solid #dde1e9;border-radius:6px;font-size:12px;font-family:inherit;outline:none" onchange="updPot('${escapeAttr(k.nik)}',${i},'nama',this.value)"><input type="text" class="inp-rp" value="${p.nilai}" data-rp="1" placeholder="Nominal" style="padding:5px 8px;border:1.5px solid #dde1e9;border-radius:6px;font-size:12px;font-family:inherit;outline:none" onchange="updPot('${escapeAttr(k.nik)}',${i},'nilai',parseRpInput(this.value))"><input value="${escapeAttr(p.ket||'')}" placeholder="Ket." style="padding:5px 8px;border:1.5px solid #dde1e9;border-radius:6px;font-size:12px;font-family:inherit;outline:none" onchange="updPot('${escapeAttr(k.nik)}',${i},'ket',this.value)"><button type="button" class="btn btn-xs btn-r"${sigajiDataAction('del-pot',{nik:k.nik,i:i})}>&#10007;</button></div>`).join(''):'<div class="font-12 text-muted p-empty-sm">Belum ada potongan.</div>';sigajiBindRpInputs(document.getElementById('pot-list'));}
function addTunj(){
...
function renderNaturaPanel(k){
  ...
  const list=k.natura||[];document.getElementById('natura-list').innerHTML=list.length?list.map((n,i)=>`<div class="nat-row ${n.kp?'kp':''}"><input class="comp-inp font-12 w-full" value="${n.nama}" onchange="updNat('${k.nik}',${i},'nama',this.value)"><input type="text" class="inp-rp comp-inp font-12 w-full" value="${n.nilai}" data-rp="1" onchange="updNat('${k.nik}',${i},'nilai',parseRpInput(this.value))"><select class="comp-inp font-11" onchange="updNat('${k.nik}',${i},'kp',this.value==='true');renderNaturaPanel(karyawan.find(x=>x.nik==='${k.nik}'))"><option value="false" ${!n.kp?'selected':''}>Tidak Kena Pajak</option><option value="true" ${n.kp?'selected':''}>Kena Pajak</option></select><button class="btn btn-xs btn-r"${sigajiDataAction('del-nat',{nik:k.nik,i:i})}>&#10007;</button></div>`).join(''):'<div class="font-12 text-muted p-empty-sm">Belum ada natura.</div>';const kp=list.filter(n=>n.kp).reduce((s,n)=>s+n.nilai,0);const nkp=list.filter(n=>!n.kp).reduce((s,n)=>s+n.nilai,0);document.getElementById('natura-total').innerHTML=list.length?`<div class="pph-box"><div class="pr-row-info"><span>Natura Tidak KP</span><span class="ct-success fw-700">${fmt(nkp)}</span></div><div class="pr-row-info"><span>Natura Kena Pajak</span><span class="ct-danger fw-700">${fmt(kp)}</span></div><div class="pph-box-tit"><span>Total</span><span>${fmt(kp+nkp)}</span></div></div>`:'';sigajiBindRpInputs(document.getElementById('natura-list'));
}
