...
/**
 * Cuti manual untuk tahun kuota `tahun`, plus hari cuti di luar kalender `tahun` yang masih
 * dalam rentang periode gaji, bila periode dianggap milik tahun itu (selesai atau bayar di tahun `tahun`).
 * Menangani periode yang menyilang tahun (mis. 29 Des 2025 - 7 Jan 2026 untuk gaji Januari 2026).
 */
function cutiManualUntukTahunDanPeriode(nik,tahun,periode){
  ...
  const endY=String(periode.end||'').substring(0,4);
  const bayY=String(periode.bayar||'').substring(0,4);
  if(endY!==y&&bayY!==y)return n;
